...
* finder search window: option+com+space
* show desktop: f11
* 最大化：control+com+f
* 关闭窗口：com+w（区别于com+q退出）
...
* 基本使用：
  * 右键给文件和目录（批量）打上tags
  * finder中点击左sidebar的tags，可以查看所有打上了对应tags的文件和目录。
  * 可以在阅读时、保存时等多种场景下添加tags。
  * 

### 程序坞dock

* launchpad：启动台，mac系统的所有程序都放在这里。
* 程序坞中最左侧和最右侧的finder和Trash无法移动，其他图标均可自定义。
* option+com+d：快速显示/隐藏程序坞
* 在system setting->desktop & dock中进行配置

### 右上状态栏

* 控制中心control center：对基础设置进行配置。
* 状态栏：控制中心拖入中的常用项。
* 通知中心：点击日期打开通知中心，也可以通过双指从trackpad右外侧划入打开。
* displays：对显示进行设置，调节亮度、深夜模式、文字大小等。
